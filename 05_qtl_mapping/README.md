...
cross-cohort QN + INT + ComBat of all modalities (normalization first, ComBat
...
| # | Script(s) | Purpose |
|---|---|---|
| 19 | `19_hcp_factors.sh`, `19a_picard_sharded.sh`, `19b_fix_missing_metrics.sh` | HCP pipeline: Picard QC (`picard_qc.py`) → pool metrics (`fix_missing_metrics.py`) → pool expression within ancestry (`pool_expression_within_ancestry.py`) → TPM > 0.1 in > 25% filter → QN + INT → connectivity-outlier removal (bicor, z < −3) → ComBat last → HCP, provisional k=15 (`combat_normalize_hcp.R`, `peer_factors.py`) |
| 20 | `20_combat_modalities.sh` | Pool + QN + INT + ComBat (last), 7 non-expression modalities (`pool_modalities_within_ancestry.py`, `combat_normalize_modalities.R`); devBrain filters (isoforms TPM > 0.1/>25%; others detected ≥ 40%); isoforms exclude expression outliers via `--exclude-samples`; splicing/IR via stage-17 pre-pooled BEDs |
...
| 25 | `25_build_covariates.py` | Covariates: PC1–5 + HCP_1–k + sex + GA + cell types; `--exclude-covariates ct_Maternal` BEFORE \|r\| > 0.9 pruning (coded; no manual edits); no cap — 25a-optimal set used in full; `--hcp-file`/`--hcp-k`/`--out-suffix` for 25a |
| 25a | `25a_optimize_hcp.sh`, `optimize_hcp_chr1.py` | HCP-count optimization (devBrain §4.2): re-estimate HCP per k ∈ {0,5,10,15,20,25,30}, map chr1 expression per k, pick k\* maximizing eGenes at Storey q ≤ 0.05 (ties → smaller k); installs k\* as `{ANC}_hcp_factors_harmonized.tsv`. Every per-k model uses the same pruned fixed covariate set (`EXCLUDE_COVARIATES`, default `ct_Maternal`; no cap); HCPs lost to correlation pruning are reported (`n_hcp_used`/`n_hcp_dropped`). Run after 23/24, before canonical 25 |
