...
| 25 | `25_build_covariates.py` | Covariates: PC1–5 + HCP_1–k + sex + GA + cell types; \|r\| > 0.9 pruning; cap ≤ 25; `--hcp-file`/`--hcp-k`/`--out-suffix` for 25a (**+ manual maternal-fraction removal afterward — see MANIFEST landmine 4**) |
| 25a | `25a_optimize_hcp.sh`, `optimize_hcp_chr1.py` | HCP-count optimization (devBrain §4.2): re-estimate HCP per k ∈ {0,5,10,15,20,25,30}, map chr1 expression per k, pick k\* maximizing eGenes at Storey q ≤ 0.05 (ties → smaller k); installs k\* as `{ANC}_hcp_factors_harmonized.tsv`. Run after 23/24, before canonical 25 |
